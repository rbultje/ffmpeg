...
static av_noinline void opn##_8tap_1d_##dir##_c(uint8_t *dst, const uint8_t *src, \
                                                ptrdiff_t stride, int w, int h, \
                                                const int8_t *filter) \
{ \
    do_8tap_1d_c(dst, src, stride, w, h, ds, filter, opa); \
}

filter_8tap_1d_fn(put, 0, v, stride)
filter_8tap_1d_fn(put, 0, h, 1)
filter_8tap_1d_fn(avg, 1, v, stride)
...
static av_noinline void opn##_8tap_2d_hv_c(uint8_t *dst, const uint8_t *src, \
                                           ptrdiff_t stride, int w, int h, \
                                           const int8_t *filterx, \
                                           const int8_t *filtery) \
{ \
    do_8tap_2d_c(dst, src, stride, w, h, filterx, filtery, opa); \
}
...
static void avg##_8tap_##type##_##sz##dir##_c(uint8_t *dst, const uint8_t *src, \
                                              ptrdiff_t stride, int h, \
                                              int mx, int my) \
{ \
    avg##_8tap_1d_##dir##_c(dst, src, stride, sz, h, \
                            vp9_subpel_filters[type_idx][dir_m - 1]); \
}

#define filter_fn_2d(sz, type, type_idx, avg) \
static void avg##_8tap_##type##_##sz##hv_c(uint8_t *dst, const uint8_t *src, \
                                           ptrdiff_t stride, int h, \
                                           int mx, int my) \
{ \
    avg##_8tap_2d_hv_c(dst, src, stride, sz, h, \
...
static av_always_inline void do_bilin_1d_c(uint8_t *dst, const uint8_t *src,
                                           ptrdiff_t stride, int w, int h,
                                           ptrdiff_t ds, int mxy, int avg)
{
    do {
        int x;
...
static av_noinline void opn##_bilin_1d_##dir##_c(uint8_t *dst, const uint8_t *src, \
                                                 ptrdiff_t stride, int w, int h, \
                                                 int mxy) \
{ \
    do_bilin_1d_c(dst, src, stride, w, h, ds, mxy, opa); \
}

bilin_1d_fn(put, 0, v, stride)
bilin_1d_fn(put, 0, h, 1)
bilin_1d_fn(avg, 1, v, stride)
bilin_1d_fn(avg, 1, h, 1)

#undef bilin_1d_fn

static av_always_inline void do_bilin_2d_c(uint8_t *dst, const uint8_t *src,
                                           ptrdiff_t stride, int w, int h,
                                           int mx, int my, int avg)
{
...
static av_noinline void opn##_bilin_2d_hv_c(uint8_t *dst, const uint8_t *src, \
                                            ptrdiff_t stride, int w, int h, \
                                            int mx, int my) \
{ \
    do_bilin_2d_c(dst, src, stride, w, h, mx, my, opa); \
}
...
static void avg##_bilin_##sz##dir##_c(uint8_t *dst, const uint8_t *src, \
                                      ptrdiff_t stride, int h, int mx, int my) \
{ \
    avg##_bilin_1d_##dir##_c(dst, src, stride, sz, h, dir_m); \
}

#define bilinf_fn_2d(sz, avg) \
static void avg##_bilin_##sz##hv_c(uint8_t *dst,  const uint8_t *src, \
                                   ptrdiff_t stride, int h, int mx, int my) \
{ \
    avg##_bilin_2d_hv_c(dst, src, stride, sz, h, mx, my); \
}
